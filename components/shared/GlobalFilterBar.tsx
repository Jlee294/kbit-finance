...
export function GlobalFilterBar({ companies, companyId, year, years }: Props) {
  ...
  return (
    <div className="sticky top-0 z-20 flex items-center gap-3 border-b border-gray-200 bg-white/95 px-6 py-2.5 shadow-sm backdrop-blur">
      <span className="text-xs font-semibold uppercase tracking-wide text-gray-400">Đang xem</span>
      <select aria-label="Công ty" value={companyId ?? ''} onChange={(e) => onCompany(e.target.value)} disabled={pending} className={`${sel} min-w-[200px] text-brand-800`}>
        <option value="">— Tất cả công ty —</option>
        {companies.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
      </select>
      <select aria-label="Năm" value={year} onChange={(e) => onYear(e.target.value)} disabled={pending} className={sel}>
        {years.map((y) => <option key={y} value={y}>Năm {y}</option>)}
      </select>
      {pending && <span className="text-xs text-gray-400">Đang cập nhật…</span>}
    </div>
  )
}
